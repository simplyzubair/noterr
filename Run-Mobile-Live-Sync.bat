...
echo.
echo Keep this window open while testing on Android.
echo Hot reload: press r in this window after code changes.
echo Hot restart: press R in this window if state needs a clean restart.
echo Quit: press q.
echo.
...
:fail
popd
echo Mobile live sync preview failed.
pause
exit /b 1
